...
def _resize_image(image: Image.Image, max_size: tuple[int, int] = (512, 512)) -> bytes:
    ...
    try:
        ...
        raise MediaProcessingError(msg) from e
...
def _try_model(
    ...
    try:
        ...
        if not model.supports_multimodal and attachments:  # type: ignore[attr-defined]
        ...
        raise ModelInvocationError(msg) from e
...
def _process_single_prompt_for_batch(
    prompt_tuple: tuple[str, Sequence[str] | None],
) -> str:
    """
...
def ask_batch(
    ...
    try:
        ...
        raise BatchProcessingError(msg) from e
